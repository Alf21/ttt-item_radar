	...
	if IsValid(ply) and ply:IsTerror() then
		if not ply:HasEquipmentItem(EQUIP_RADAR) and ply:HasEquipmentItem(EQUIP_RADAR_CUSTOM) then
			...
			for _, p in pairs(scan_ents) do
				if IsValid(p) and ply ~= p and (
					p:IsPlayer() and p:IsTerror() and not p:GetNWBool("disguised", false) or not p:IsPlayer()
				) then
					...
					elseif not TTT2 and role ~= ROLE_INNOCENT and role ~= ply:GetRole()
					or TTT2 and role ~= ROLE_INNOCENT
					then
						if not TTT2 then
							role = ROLE_INNOCENT
						else
			...
			for _, tgt in pairs(targets) do
				net.WriteUInt(tgt.role, TTT2 and ROLE_BITS or 2)
